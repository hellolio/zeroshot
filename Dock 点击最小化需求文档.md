...
为 zeroshot 新增一个能力：**单击 Dock 上「当前前台已激活 app」的图标，最小化该 app 在当前桌面空间的全部窗口**（Windows 任务栏式的单击最小化）。恢复窗口完全交给 macOS 原生行为（再点一次 Dock 图标即恢复），本模块只做"最小化"这一个方向。
...
| 场景 | 行为 |
|------|------|
| 前台 app（含任意第三方 app）窗口可见、非全屏，单击其 Dock 图标 | 本模块最小化该 app 在当前空间的所有可见窗口 |
...
- **FR-2.4** 满足条件 → 将这些窗口的 `kAXMinimized` 置 true（窗口滑入 Dock）
...
2. 任意前台 app 单击其 Dock 图标 → 当前空间全部可见非全屏窗口滑入 Dock；再次单击 → macOS 正常恢复。
